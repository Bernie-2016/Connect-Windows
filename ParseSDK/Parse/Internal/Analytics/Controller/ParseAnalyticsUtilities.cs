...
namespace Parse.Internal.Analytics.Controller
{
	public static class ParseAnalyticsUtilities
	{
		public static string MD5DigestFromPushPayload(object payload)
		{
			string content;
			if (payload == null)
			{
			...
			else if (payload is IDictionary<string, object>)
			{
				IDictionary<string, object> array = (IDictionary<string, object>) payload;

				List<string> components = new List<string>();
				foreach (string key in array.Keys.OrderBy(x => x))
				{
					components.Add(key);
					...
					{
						components.Add(string.Join("", (List<object>)value));
					}
					else
					{
				...
				content = string.Join("", components);
			}
			else
			{
			...
			return MD5HashFromString(content);
		}

		public static string MD5HashFromString(string input)
		{
			byte[] hash = MD5.GetHash(input, Encoding.UTF8);
